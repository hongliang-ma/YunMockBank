 ...
 * 8583长度位添加
 * @author jun.qi
 * @version $Id: MsgLengthBuild.java, v 0.1 2012-8-3 下午01:46:33 jun.qi Exp $
 */
public class MsgLengthBuild extends ComponetHandler {
    ...
    public void addLengthField(TransferData data, TransferData localTransferData) throws Exception {
        ...
        String inter = (String) localTransferData.getObject("MsgLengthBuild" + CD_INTER);
        Matcher m = p.matcher(inter);
        inter = inter.substring(m.regionStart() + 1, m.regionEnd() - 1);

        int len = Integer.parseInt((String) localTransferData.getObject("MsgLengthBuild"
                                                                        + CD_LENGTH));
        String encoding = (String) localTransferData.getObject("MsgLengthBuild"
                                                               + CD_LENGTH_ENCODING);
    ...
    public void takeLengthField(TransferData data, TransferData localTransferData) throws Exception {
        ...
        content = (byte[]) data.getProperties().get((DataMapDict.MSGBODY));
    ...
    protected void process(TransferData data, TransferData localTransferData)
        ...
        String obj = (String) localTransferData.getObject("MsgLengthBuild" + CD_REVERSE);
        try {
            Boolean revs = Boolean.valueOf(obj);
            if (revs) {
            ...
            ExceptionUtil.caught(e, " 8583长度位添加异常");
